...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (cards.length === 0) {
      cards.push({name: 'Click to change name', weapon: 'blunt', style: 'power', id: cards.length+1});
    }
    this.setState({cards});
  }
  rememberCard(card) {
  ...
  addCard() {
    let cards = this.getCards();
    cards.push({name: 'Click to change name', weapon: 'blunt', style: 'power', id: this.generateId()});
    localStorage.cards = JSON.stringify(cards);
    this.setState({cards});
  }
  removeCard(id) {
